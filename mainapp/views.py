...
from .serializers import BetSerializer, TransactionSerializer,BrokerCreateSerializer, UserCreateSerializer, UserProfileListSerializer, AddPointsSerializer
        ...
        if serializer.is_valid():
            if serializer.data["gameID"] == 1 or serializer.data["gameID"] == 2:
                ...
                if voter_user.points >= serializer.data['amount']:
                    ...
                    transaction = Transaction(sender=request.user.profile, receiver=request.user.profile, amount=serializer.data['amount'], transaction_type=2)
                    voter_user.save()
                    entry.save()
                    transaction.save()
